  ...
  return (
    <AnimatePresence>
      {isVisible && (
        ...
        >
          <div className="mb-2">
            <h4 className="text-coral font-bold text-base uppercase tracking-wider text-start">
              {t("home.service_section.tags_title")}
            </h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-0 sm:gap-3 md:gap-3 lg:gap-3 xl:gap-3 mt-3">
            {tags.map((tag, index) => (
              ...
              >
                ...
                <span className="leading-relaxed text-gray-700 dark:text-gray-300 text-base text-start">
                  {tag.text}
                </span>
              </motion.div>
            ))}
      ...
      >
        <div
          className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 transition-colors duration-300 ${
            isHovered ? "bg-[#FF6B6B]" : "bg-[#F9F9FB] dark:bg-gray-700"
          }`}
        >
          <Icon
            size={36}
            className={`transition-colors duration-300 ${
              isHovered ? "text-white" : "text-coral"
            }`}
          />
        </div>

        <h3 className="text-xl font-bold uppercase mb-4 tracking-wide text-gray-900 dark:text-white">
          {title}
        </h3>
