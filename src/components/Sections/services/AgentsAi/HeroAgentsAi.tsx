...
export const HeroAgentsAiSection = () => {
    ...
    const benefitsList = /* t( */'ai_agents.hero_section.benefits', { returnObjects: true }) as string[];
    ...
    return (
        <section className="pt-8 sm:pt-8 md:pt-10 lg:pt-15 xl:pt-16 w-full overflow-hidden flex items-center text-gray-600 dark:text-gray-300 leading-relaxed">
            ...
            {/* Floating elements */}
            <motion.div className="absolute left-[5%] top-[15%] h-32 w-32 rounded-full bg-[#FF6B6B]/5" animate={{ y: [0, -10, 0], transition: { duration: 6, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" } }} />
            <motion.div className="absolute bottom-[20%] right-[10%] h-24 w-24 rounded-full bg-[#FF6B6B]/10" animate={{ y: [0, 15, 0], transition: { duration: 7, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" } }} />
            <motion.div className="absolute left-[15%] top-[60%] h-16 w-16 rounded-full bg-[#FF6B6B]/5" animate={{ y: [0, -15, 0], transition: { duration: 8, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" } }} />

            {/* Dots */}
            <div className="absolute inset-0 -z-10 opacity-20 pointer-events-none">
                {Array.from({ length: 20 }).map((_, i) => (
                    <div key={i} className="absolute h-1 w-1 rounded-full bg-coral" style={{ top: `${Math.random() * 100}%`, left: `${Math.random() * 100}%`, opacity: Math.random() * 0.5 + 0.3 }} />
                ))}
            </div>
            ...
            <div className="container mx-auto relative z-10">
                <div className="grid gap-4 md:gap-8 lg:gap-16 xl:gap-20 grid-cols-1 lg:grid-cols-2 items-center relative">
                    ...
                    <div className="relative z-20 space-y-8">
                        ...
                        >
                            {benefitsList.map((benefit, index) => (
                                <BenefitChip key={index} icon={benefits[index]?.icon} text={benefit} delay={index * 0.1} />
                            ))}
                        </motion.div>
                    ...
                    <div className="w-full h-800px sm:h-800px md:h-[85vh] lg:h-800px xl:h-800px overflow-hidden absolute inset-0 opacity-30 z-0 lg:relative lg:inset-auto lg:opacity-100 lg:z-auto">
                        {theme === 'light' ? (
                            <>
                                <img src="/images/vectors/light-top-gradiant.webp" alt="Degradado arriba" className="pointer-events-none absolute top-0 left-0 z-10 w-full h-1/3 object-cover" />
                                <img src="/images/vectors/light-bottom-gradiant.webp" alt="Degradado abajo" className="pointer-events-none absolute bottom-0 left-0 z-10 w-full h-1/3 object-cover" />
                            </>
                        ) : (
                            <>
                                <img src="/images/vectors/dark-shadow-top.webp" alt="Degradado arriba" className="pointer-events-none absolute top-0 left-0 z-10 w-full h-1/3 object-cover" />
                                <img src="/images/vectors/dark-shadow-bottom.webp" alt="Degradado abajo" className="pointer-events-none absolute bottom-0 left-0 z-10 w-full h-1/3 object-cover" />
                            </>
                        )}
